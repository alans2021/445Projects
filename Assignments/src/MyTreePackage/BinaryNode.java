...
public class BinaryNode<T>
{
   ...
   public BinaryNode<T> copy()
   {
	...
	{
		if(node != null) //Check to see if node exists
		{
			if(node.leftChild != null) //Check to see if left child exists
			{
				...
				{
					if(node.leftChild.hasLeftChild() && !node.rightChild.hasLeftChild())
						return false;
	...
	private boolean recBalanced(BinaryNode<T> node, int k)
	{
		if(node != null)
		{
			int lHeight; int rHeight;
			
			if(node.hasLeftChild()) //Get height of left subtree
				lHeight = node.leftChild.getHeight();
			else
				lHeight = 0;
			
			if(node.hasRightChild()) //Get height of right subtree
				rHeight = node.rightChild.getHeight();
			else
				rHeight = 0;
			
			if( Math.abs(lHeight - rHeight) <= k ) //If current tree is balanced, check if both subtrees are balanced
				return recBalanced(node.leftChild, k) && recBalanced(node.rightChild, k);
			else
				return false;
		}
		return true; //Returns true since a single node is 'balanced', so children will both be null
	} 
