...
public class JRHanoi extends JFrame implements Runnable
{
     ...
     public JRHanoi()
     {
        	...
        	playGame.setFont(new Font("TimesRoman", Font.BOLD, 40));
            runDemo = new JButton("Run Demo");
            runDemo.setFont(new Font("TimesRoman", Font.BOLD, 40));
            ...
            for (int i = 0; i < 3; i++)
            {
                 ...
                 towerButtons[i].setFont(new Font("TimesRoman", Font.BOLD, 40));
			...
			info.setFont(new Font("TimesRoman", Font.BOLD, 40));
 ...
 // records and the run-time stack, as we discussed in lecture.
     ...
     public void solveHanoi(int currsize, int start, int middle, int end)
     {
            ...
            {					// to end directly
                  ...
                  moves++;
					...
					// 3) recursively move N-1 disks from middle to end
			{
     ...
     private class ButtonHandler implements ActionListener
     {
            public void actionPerformed(ActionEvent e)
            {
                 if (e.getSource() == runDemo)
                 {
                       ...
                       info.setText("Running demo...please wait until it finishes");
     ...
     private class Tower
     {
         public final int MAXWIDTH = 240, HEIGHT = 50;
         ...
         public void fill()
         {
             ...
             for (int i = 0; i < disks.length; i++)
             {
                    ...
                    int y = 600 - ((i+1) * HEIGHT);
                    disks[i] = new Rectangle2D.Double(x, y, width, 50);
                    width = width - 20;
             }
             number = disks.length;
         }
         ...
         public void add(Rectangle2D curr)
         {
             ...
             int x = xoffset + (MAXWIDTH - width)/2;
             int y = 600 - ((number+1) * HEIGHT);
             curr.setFrame(x, y, width, 50);
             disks[number] = curr;
             number++;
         }
